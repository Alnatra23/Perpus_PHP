...
if($_GET['id']){
    ...
    if(strtotime($dt_pinjam['tgl_kembali'])>=strtotime(date('Y-m-d'))){
        $denda=0;
    } else{
    ...
    mysqli_query($conn, "insert into pengembalian_buku (id_peminjaman_buku, tgl_kembali,denda) value('".$id_peminjaman_buku."','".date('Y-m-d')."','".$denda."')");
